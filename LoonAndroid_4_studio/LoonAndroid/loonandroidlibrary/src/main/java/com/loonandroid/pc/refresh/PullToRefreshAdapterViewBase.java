...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView> extends PullToRefreshBase<T> implements OnScrollListener {
	...
	/**
	 * Pass-through method for {@link PullToRefreshBase#getRefreshableView() getRefreshableView()}. {@link AdapterView#setAdapter(android.widget.Adapter)} setAdapter(adapter)}. This is just for convenience!
	 * 
	 * @param adapter
	 *            - Adapter to set
	 */
	...
	protected void handleStyledAttributes(TypedArray a) {
		...
		mShowIndicator = LoonConstant.pullToRefresh.ptrShowIndicator || !isPullToRefreshOverScrollEnabled();
	}
	...
	private void addIndicatorViews() {
		...
		if (mode.showHeaderLoadingLayout() && null == mIndicatorIvTop) {
			...
			params.rightMargin = Handler_System.dip2px(LoonConstant.pullToRefresh.right_padding);
