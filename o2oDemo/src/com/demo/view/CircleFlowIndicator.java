...
public class CircleFlowIndicator extends View implements FlowIndicator, AnimationListener {
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int iLoop = 0; iLoop < count; iLoop++) {
			canvas.drawCircle(leftPadding + mRadius + (iLoop * spacing) + centeringOffset, getPaddingTop() + mRadius, mRadiusInactive, mPaintInactive);
		}
		float cx = 0;
		if (mSnap) {
		...
		canvas.drawCircle(leftPadding + mRadius + cx + centeringOffset, getPaddingTop() + mRadius, mRadiusActive, mPaintActive);
	}
