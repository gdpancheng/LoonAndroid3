...
/**
 * A horizontally scrollable {@link ViewGroup} with items populated from an {@link Adapter}. The ViewFlow uses a buffer to store loaded {@link View}s in. The default size of the buffer is 3 elements on both sides of the currently visible {@link View}, making up a total buffer size of 3 * 2 + 1 = 7. The buffer size can be changed using the {@code sidebuffer} xml attribute.
 * 
 */
public class ViewFlow extends AdapterView<Adapter> {
	...
	/**
	 * Extra return value from obtainView: tells you whether the item it returned on the last call was recycled rather than created by the adapter. This is a member because getting a second return value requires an allocation.
	 */
...
//			heightSize |= (childState & MEASURED_STATE_MASK);
	...
	protected void onScrollChanged(int h, int v, int oldh, int oldv) {
		...
		if (mIndicator != null) {
			/*
			 * The actual horizontal scroll origin does typically not match the perceived one. Therefore, we need to calculate the perceived horizontal scroll origin here, since we use a view buffer.
			 */
